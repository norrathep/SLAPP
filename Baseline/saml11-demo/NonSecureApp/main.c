...
const uint8_t ecc_private_key[32] = {0x89, 0x8b, 0xc, 0xde, 0xab, 0xb5, 0x86, 0x97,
...
int verify_req(pox_call_t *pox_fn, uint8_t *token) {
	...
	/*
	char ecc_sig[72] = {0};
	t =  uECC_sign(prv_sk, hash.hash, SHA256_DIGEST_SIZE, ecc_sig, curve);
	
	uart_puts("Produce siggg: ");
	print_token(ecc_sig, 64);
	uart_puts("\n");
	uart_puts("Signing succeeds? ");
	uart_puti(t);
	uart_puts("\n");*/
	
	t = uECC_verify(vrf_pk, hash.hash, SHA256_DIGEST_SIZE, token, curve);
	uart_puts("Verificationd succeeds? ");
	uart_puti(t);
	uart_puts("\n");
...
int main(void)
{
	...
	if(sign) {
		uart_puts("START SIGN\n");
		
		int size = 32;
		uart_puts("Message\n");
		uint8_t msg[100] = {}; 
		uart_get_bytes(msg, size);
		oakSphincSign(msg, size);
		
	} else {
	
		uart_puts("START VERIFY\n");
		int size = 32; 
		uart_puts("Message\n");
		uint8_t msg[100] = {}; //"helloworld";
		uart_get_bytes(msg, size);
	
		//int size = 10;
		oakSphincVerify(msg, size);
	}
	
	uart_puts("Finish running sphincs\n");
	return;*/
	// TODO: Sphincs end
