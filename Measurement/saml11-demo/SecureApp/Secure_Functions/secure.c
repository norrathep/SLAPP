...
// Correspond to the initial state: uint8_t state = 10;
...
#define STACK_BASE 0x20000328 // ED
...
void pox(pox_call_t *pox_fn, uint8_t *pox_token) {
	...
	//int key_length = 16;
	//for(int i=0; i<key_length; i++) key[i] = 0x0b;
	//int att_size = 8;
	...
	for(;;) {
		...
		if (n == 0) break;
		printsig_function psig = (printsig_function)(0x863c);
		psig(pox_token, n);
	}
	#endif
